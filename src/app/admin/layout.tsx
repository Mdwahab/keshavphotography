  ...
  return (
    <div className="flex h-screen bg-[var(--background)] text-[var(--foreground)] overflow-hidden">
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden z-10 w-full">
        ...
        {/* Top Header */}
        <header className="h-16 border-b border-[var(--border-color)] glass-panel flex items-center justify-between px-4 md:px-8 shrink-0 z-10">
          <div className="flex items-center gap-4">
            <button 
              className="md:hidden text-[var(--muted-text)] hover:text-[var(--foreground)]"
              onClick={() => setMobileMenuOpen(true)}
            >
              <Menu size={24} />
            </button>
            <h1 className="font-cinzel text-lg md:text-xl text-[var(--foreground)]">Admin Dashboard</h1>
          </div>
          <div className="font-space text-[10px] md:text-xs tracking-widest text-[var(--muted-text)] uppercase">
            Logged in as: <span className="text-[#D4AF37]">keshav0101</span>
          </div>
        </header>

        {/* Content Scrollable Area */}
        <div className="flex-1 overflow-y-auto relative custom-scrollbar p-6 md:p-12 w-full">
          {/* Cinematic ambient glow for dashboard */}
          <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-[#D4AF37]/5 blur-[100px] rounded-full pointer-events-none" />
